...
function AnnouncementModal({ announcement, onClose }) {
  ...
  return (
    <ModalOverlay>
      <ModalCard role="dialog" aria-modal="true" aria-labelledby="announcement-title">
        ...
        <ModalHeader>
          ...
          <ModalDescription>{announcement.description}</ModalDescription>
        </ModalHeader>
            ...
            {(optionSlides.length ? optionSlides : [[]]).map((slide, slideIndex) => (
              <OptionList key={`slide-${slideIndex}`}>
                {slide.length ? (
                  ...
                  <EmptyState>No options available for this announcement.</EmptyState>
                )}
              </OptionList>
            ))}
          </OptionsTrack>
